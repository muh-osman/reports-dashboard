...
export default function PayPassengerCheck() {
  ...
  // Fixed data value
  const [plan, setPlan] = useState("المسافر");
  const [model, setModel] = useState(carType === "sedan" ? "سيدان" : carType === "suv" ? "دفع رباعي" : "فارهة");
  ...
  const [airBagCheckPrice, setAirBagCheckPrice] = useState(carType === "sedan" ? 100 : carType === "suv" ? 150 : 200);
    ...
    if (carType) {
      ...
      if (priceObj) {
        ...
        setCheckedAdditionalServices((prev) => {
          const hasAirBagCheck = prev.includes("فحص الإيرباق");

          if (isAirBagCheckChecked && !hasAirBagCheck) {
            // Add "فحص الإيرباق" if checked and not already in array
            return [...prev, "فحص الإيرباق"];
          } else if (!isAirBagCheckChecked && hasAirBagCheck) {
            // Remove "فحص الإيرباق" if unchecked and present in array
            return prev.filter((item) => item !== "فحص الإيرباق");
          }
          // Return unchanged if no change needed
          return prev;
        });

        // Add AirBag Check price if checked
        if (isAirBagCheckChecked) {
          calculatedTotal += airBagCheckPrice;
        }
        ...
        <div className={style.details_order_table}>
          ...
          <div>
            <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-start", border: "none", padding: 0 }}>
              <p>
                {t("PayPurchaseCheck.inspection")} {t("PayPurchaseCheck.passenger")}
              </p>
              <p style={{ fontSize: "12px" }}>{model}</p>
            </div>
            <p style={{ display: "flex", alignItems: "center", gap: "3px", fontWeight: 700 }}>
              {Math.trunc(basePrice)} <CurrencyIcon fill="#747a79" style={{ width: "20px", height: "20px" }} />
            </p>
          </div>
          ...
          <div>
            <FormControlLabel
              control={
                <Checkbox
                  checked={isAirBagCheckChecked}
                  onChange={(e) => setIsAirBagCheckChecked(e.target.checked)}
                  sx={{
                    color: "primary.main",
                    padding: 0,
                    "&.Mui-checked": {
              ...
              label={<Typography variant="body1">{t("PayPurchaseCheck.airBagCheck")}</Typography>}
              sx={{
                margin: 0,
                width: "100%",
              }}
            ...
            <p style={{ textDecoration: isAirBagCheckChecked ? "none" : "line-through", display: "flex", alignItems: "center", gap: "3px", fontWeight: 700 }}>
              {airBagCheckPrice} <CurrencyIcon fill="#747a79" style={{ width: "20px", height: "20px" }} />
            </p>
          </div>
